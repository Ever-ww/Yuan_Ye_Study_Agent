...
class StateController:
    ...
    def _connection(self):
        ...
        guard = (self.write_gate.database_transaction(read_only_allowed=True)
        ...
        with guard:
            ...
            def authorize(action, *unused):
                nonlocal write_admitted
                if action in {sqlite3.SQLITE_INSERT, sqlite3.SQLITE_UPDATE, sqlite3.SQLITE_DELETE,
                              sqlite3.SQLITE_CREATE_TABLE, sqlite3.SQLITE_DROP_TABLE,
                              sqlite3.SQLITE_ALTER_TABLE} and not self._initializing and not write_admitted:
                    try:
                        if self.write_gate:
                            self.write_gate.check_mutation_admission()
                        # _connection is synchronous and holds the same gate lock
                        # throughout commit/rollback: admission cannot change
                        # between statements. Avoid reopening the control DB for
                        # every INSERT/UPDATE generated by this transaction.
                        write_admitted = True
                    except Exception as exc:
                        denied.append(exc)

...
- `legacy.py`：旧同步 `Agent`、`AgentConfig`、`ReActAgent`、同步 `ToolRegistry` 的唯一真实实现。
- `runtime.py`：新异步 `AgentRuntime` 主循环，只依赖正式的 Harness 组件。
- `subagents.py`：发现和解析 Markdown 子代理定义。
- `teams.py`：团队任务 DAG、领取状态与邮箱持久化。
- `agent.py`、`react_agent.py`、`tool_registry.py`、`agents.py`：仅保留历史导入路径的兼容转发，不再承载业务实现。

旧代码无需立即迁移；新代码应分别从 `Agent.legacy`、`Agent.subagents` 或 `Agent.teams`
导入具体实现。包顶层 `from Agent import Agent` 仍表示旧同步 API，而推荐的新入口是
...
| Agent 兼容层 | `Agent/legacy.py` | 集中旧同步 Agent、ReAct 循环、结果类型和同步工具注册表真实实现 |
| 兼容转发 | `Agent/agent.py`、`react_agent.py`、`tool_registry.py`、`agents.py` | 只转发旧导入路径，不再承载业务逻辑 |
...
- `legacy.py` 是同步 `Agent`、`AgentConfig`、`ReActAgent`、`Step`、旧 `AgentResult`
  和同步 `ToolRegistry` 的唯一实现位置。
- `agent.py`、`react_agent.py`、`tool_registry.py` 是薄兼容 shim。
...
| 文件 | 覆盖重点 |
| --- | --- |
| `tests/test_harness.py` | 配置、公共 API、冷导入、权限优先级、URL/IP、Memory/Corpus、Runtime、rewind、Cron、Teams |
